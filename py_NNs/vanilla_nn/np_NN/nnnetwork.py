...
class Network:
    def __init__(self):
        ...
        self.W1 = np.random.randn(15, 784) * np.sqrt(2 / 784)
        self.b1 = np.zeros((15, 1))
        self.W2 = np.random.randn(10, 15) * np.sqrt(2 / 15)
        self.b2 = np.zeros((10, 1))
        
        # Initialize gradients to zeros
        self.dW1 = np.zeros((15, 784))
        self.db1 = np.zeros((15, 1))
        self.dW2 = np.zeros((10, 15))
    ...
    def create(self, Y): # Passing in a 1 x 41000 matrix (41000 columns, 1 row)
        ...
        matrix_Y[np.arange(Y.size), Y] = 1 # Looks like array([0,1,2,..., 40997, 40998, 40999]), then indexing the Y values aswell at each column, changing that value to 1
